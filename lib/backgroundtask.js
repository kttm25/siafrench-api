            ...
            for (let i = 0; i < result.hosts.length; i++) {
                if (result.hosts[i]['acceptingcontracts']) {
                    if (topHost.N1 != {} && topHost.N1.totalstorage < result.hosts[i]['totalstorage']) {
                        ...
                        topHost.N4 = topHost.N4;
                        topHost.N3 = result.hosts[i];
                    }
                    else if (topHost.N4 != {} && topHost.N4?.totalstorage < result.hosts[i]['totalstorage']) {
                        topHost.N4 = result.hosts[i];
                    }
                }
            }
...
async function networkBlockAnalysis() {
    siad.siaNetworkData.consensus().then(async function (result1) {
        ...
        var AllMinersAddress = [];
        var TopMinersAddress = [];
        ...
        for (let i = 0; i <= result1.height; i++) {
            ...
            await siad.siaNetworkData.consensusblock(i.toString()).then(async (result2) => {
                if (result2.minerpayouts.length > 0) {
                    ...
                    result2.minerpayouts.forEach((minerpayout, index2) => {
                        if (AllMinersAddress.length == 0) {
                            AllMinersAddress.push({ minerHash: minerpayout.unlockhash, iteration: 1 });
                        } else {
                            let update = false;
                            AllMinersAddress.forEach((miner, index) => {
                                if (miner.minerHash === minerpayout.unlockhash){
                                    AllMinersAddress[index].iteration += 1;
                                    update = true
                                }
                            })
                            if(!update)
                ...
                //Order all miner by iteration
                AllMinersAddress.sort(( a, b ) => b.iteration - a.iteration)

                //get Top Miners
                TopMinersAddress = AllMinersAddress.filter(miner => miner.iteration > 1)

                //console.log(AllMinersAddress)
                ...
                if (count == result1.height) {
                    ...
                    if (!process.env.DATABASE_ENABLE) {
                        await fs.writeFile(process.env.FILE_DATA_LOCATION, JSON.stringify({
                            ...
                            //"allMinersAddress" : AllMinersAddress
                        }), (err) => {
                        ...
                        await InsertDatabaseData({
                            ...
                            /*allMiners: {
                                allMiningAddress: AllMinersAddress,
                                timestamp : dateNow.getTime()
                            },*/
                            topMiners:{
                                topMiningAddress: TopMinersAddress,
                                timestamp : dateNow.getTime()
                            },
...
exports.doInBackground = async function doInBackground() {
    ...
    //Start background on launch app
